/**
 * Canonical medicine categorisation and the storage condition each category
 * requires. Used to (a) offer a real, controlled category list, (b) derive the
 * storage condition a medicine needs, and (c) check that a lot is filed in a
 * physically suitable storage unit.
 *
 * Conditions follow standard pharmacy practice:
 *   - cold        → réfrigéré, 2–8 °C  (vaccins, insulines, biologiques, sérums)
 *   - ambient     → température ambiante contrôlée, 15–25 °C (la majorité)
 *   - controlled  → armoire sécurisée/verrouillée (stupéfiants, psychotropes)
 *
 * Sources: WHO/MSF drug storage guidance, USP controlled-room-temperature
 * (15–25 °C), cold-chain 2–8 °C for biologics/vaccines/insulin.
 */
...
export interface ConditionMeta {
  label: string;       // French label shown in the UI
  short: string;       // compact label / chip
  tempRange: string;   // human-readable target range
  minTemp?: number;
  maxTemp?: number;
  /** Storage-unit types that physically satisfy this condition. */
...
export interface CategoryDef {
  name: string;                 // canonical French category name
  condition: StorageCondition;
  forms?: string;               // typical galenic forms (hint)
}

/**
 * Canonical therapeutic categories. The list is intentionally compact and
 * pharmacy-oriented; the condition is the DEFAULT requirement for the class.
 */
export const MEDICINE_CATEGORIES: CategoryDef[] = [
  ...
  // Controlled substances (secure cabinet)
...
// Keyword rules so an ARBITRARY (legacy / free-text) category still resolves to
// the right condition. Order matters: controlled and cold win over ambient.
...
/**
 * The storage condition a medicine of this category requires. Falls back to a
 * keyword scan so legacy free-text categories still resolve sensibly; default
 * is `ambient`.
 */
...
/** Storage-unit types that physically satisfy a medicine's condition. */
export function acceptableStorageTypes(condition: StorageCondition): StorageType[] {
  return CONDITION_META[condition].storageTypes;
}

/** True when a unit of `storageType` is suitable for a medicine needing `condition`. */
export function storageSuitsCondition(condition: StorageCondition, storageType?: StorageType): boolean {
  ...
  return acceptableStorageTypes(condition).includes(storageType);
}
