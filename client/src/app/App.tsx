          ...
          {scannedMed ? (
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-border space-y-3">
                <input type="text" placeholder="Batch Number" className="w-full px-4 py-2 bg-input-background border border-border rounded-lg" />
                <input type="number" placeholder="Quantity" className="w-full px-4 py-2 bg-input-background border border-border rounded-lg" />
                <input type="date" className="w-full px-4 py-2 bg-input-background border border-border rounded-lg" />
                <input type="text" placeholder="Storage Location" className="w-full px-4 py-2 bg-input-background border border-border rounded-lg" />
              </div>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <AlertCard title="Expiring in 30 Days" count={expiring.length} description="Review and plan disposal" variant="warning" icon={Clock} />
        <AlertCard title="Expiring in 90 Days" count={28} description="Monitor closely" variant="info" icon={AlertTriangle} />
      </div>
      ...
      <div className="bg-card rounded-lg border border-border p-6">
        ...
        <div className="space-y-3">
          {expired.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-4 border border-destructive/20 bg-destructive/5 rounded-lg">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Batch: {item.batch} · Expired: {item.expiry}</p>
                </div>
              </div>
            ...
            {users.map(u => (
              <tr key={u.id} className="hover:bg-muted/50">
                ...
                <td className="px-6 py-4">
                  <span className="px-2 py-1 rounded-full text-xs font-semibold bg-success/10 text-success">{u.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
...
function AlertCard({ title, count, description, variant, icon: Icon }: any) {
  ...
  return (
    <div className={`rounded-lg border p-6 ${variants[variant as keyof typeof variants]}`}>
